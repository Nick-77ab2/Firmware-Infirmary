Shader "Hidden/Shader/Sobel"
{
	...
	float SobelSampleDepth(float2 uv, float offsetU, float offsetV) {
		...
		return Sobel_Scharr(abs(topLeft - center), abs(top - center),		   abs(topRight - center),
							 ...
							 abs(bottomLeft - center), abs(bottom - center), abs(bottomRight - center));
	}
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
		...
		float3 finalColor = lerp(sourceColor, _Color, outlineIntensity * _Intensity);

		//sobel effect debug visualizer
